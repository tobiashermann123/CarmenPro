...
**Step 0 — EUR/USD Exchange Rate**

Before fetching stock data, retrieve the current EUR/USD exchange rate. This is required because CarmenPro is a EUR-denominated fund (~€100K) and ALL price levels (entry, targets, stops) must be expressed in both native currency AND EUR.

Search query: `"EUR USD exchange rate today 2026"`

Extract: the current EUR/USD rate (e.g., 1.0850). Store this as `EUR_USD_RATE` for use throughout the analysis.

- If the stock trades in USD: EUR price = USD price ÷ EUR_USD_RATE
- If the stock trades in EUR (e.g., LVMH on Euronext): already EUR-denominated, note this explicitly, no conversion needed
- If the stock trades in another currency (GBP, JPY, etc.): convert via USD as the bridge

Include the EUR_USD_RATE in the DISCOVERY_BRIEF so all subagents are aware of it.

**Step 1 — Current Price & Market Context**

Use WebSearch to find:
- Current stock price for TICKER (note the native currency)
- Today's price change (dollar and percentage)
- Market cap and cap category (Large/Mid/Small/Micro)
- Average daily volume
- 52-week high and 52-week low (in both native currency and EUR)
...
### Key Levels
- Entry Zone: $X - $X (€X - €X at EUR/USD [rate from discovery brief])
- Stop Loss: $X / €X (X% below entry)
- Target 1: $X / €X (X% upside)
- Target 2: $X / €X (X% upside)
...
[2-3 paragraph synthesis of the entire analysis. What is this company, what is the setup, and what is the verdict? Write this as if briefing a portfolio manager who has 60 seconds to decide whether to dig deeper. ALWAYS include EUR-equivalent prices for all key price levels mentioned (entry zone, targets, stop loss) — CarmenPro is a EUR-denominated fund. Format: "$X.XX (€X.XX)". State the EUR/USD rate used, e.g. "At EUR/USD 1.0850, the entry zone of $95–$108 equates to approximately €88–€99."]
...
> EUR/USD rate used: X.XXXX (fetched at time of analysis — verify before trading)

| Parameter | USD | EUR | Notes |
|-----------|-----|-----|-------|
| Entry Zone | $X – $X | €X – €X | [reasoning] |
| Stop Loss | $X | €X | [X% risk from entry] |
| Target 1 | $X | €X | [X% reward, conservative] |
| Target 2 | $X | €X | [X% reward, aggressive] |
| Risk/Reward | X:1 | — | [at midpoint entry to T1] |
| Position Size | X% of portfolio | ~€X,XXX | [CarmenPro €100K fund basis] |
...
| [factor 4] | [factor 4] |
| [factor 5] | [factor 5] |
| **Bull Target: $X (€X)** | **Bear Target: $X (€X)** |
...
9. ALWAYS fetch the EUR/USD rate in Phase 1 Step 0 and include it in the Discovery Brief.
10. ALWAYS express every price level (current price, entry zone, stop loss, targets, 52W high/low) in BOTH native currency AND EUR. Format: "$X.XX (€X.XX)". This is non-negotiable for the CarmenPro EUR-denominated fund.
11. ALWAYS state the EUR/USD rate used at the top of the Entry/Exit Strategy section so the user can verify it.

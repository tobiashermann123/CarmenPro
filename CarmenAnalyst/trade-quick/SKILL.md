...
Run 4 focused WebSearch queries in parallel (launch all in one message):

**Query A — Price & Performance**
`"<TICKER> stock price today market cap P/E 52 week high low 2026"`

From this, extract:
- Current price and today's dollar/percentage change (note native currency)
- Market cap
- 52-week high and 52-week low
- P/E ratio (trailing)
- Average volume

**Query D — EUR/USD Exchange Rate**
`"EUR USD exchange rate today 2026"`

From this, extract:
- Current EUR/USD rate (e.g., 1.0850)
- Use this to convert all USD price levels: EUR = USD ÷ rate
- If the stock is EUR-denominated (Euronext, etc.), mark it as EUR-native — no conversion needed
- Record the rate for display in the output so the user can verify it before trading
...
============================================================
  ...
  <DATE> | AI Trading Analyst | CarmenPro EUR Fund
============================================================

  Price:    $X.XX / €X.XX  (today: +/-$X.XX / +/-X.XX%)
  Mkt Cap:  $X.XB  |  P/E: X.X  |  Sector: <sector>
  52W:      $X.XX / €X.XX (low) — $X.XX / €X.XX (high)  [X% from high]
  Volume:   X.XM  (avg: X.XM)
  FX:       EUR/USD X.XXXX  [USD prices ÷ rate = EUR equivalent]
...
------------------------------------------------------------
  ...
  KEY LEVELS (USD / EUR):
  Resistance: $X.XX / €X.XX  |  Support: $X.XX / €X.XX
  Analyst Target: $X.XX / €X.XX
...
**EUR Note:** For EUR-native stocks (e.g., Euronext listings), show `€X.XX` only in the Price and Key Levels rows — omit the USD column and replace the FX line with `Currency: EUR-native (no conversion needed)`.
...
1. Launch all 4 WebSearch queries (A, B, C, D) in the SAME message for parallel execution.
...
5. Do NOT over-research — 4 searches is enough. Resist the urge to dig deeper.
6. If a search returns limited data, work with what you have rather than running additional searches.
7. The EUR/USD query (D) should resolve quickly — if it fails, use the most recently known rate and flag it as "rate unverified".
...
============================================================
  ...
  FX: EUR/USD X.XXXX
============================================================
  Ticker | USD Price | EUR Price | Signal | P/E  | Key Factor
  AAPL   | $X.XX    | €X.XX    | BUY    | X.X  | [one-line]
  MSFT   | $X.XX    | €X.XX    | HOLD   | X.X  | [one-line]
  GOOG   | $X.XX    | €X.XX    | BUY    | X.X  | [one-line]
============================================================
